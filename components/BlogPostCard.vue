<template>
  <div
    class="card group h-full flex flex-col hover:shadow-lg transition-all duration-300"
  >
    <div class="relative overflow-hidden aspect-video">
      ...
      <div class="absolute top-4 left-4" v-if="post.category">
        <span
          class="inline-block px-3 py-1 text-xs bg-primary-600 text-white rounded-full"
        >
          {{ post.category }}
        </span>
      </div>
    ...
    <div class="p-6 flex-grow flex flex-col">
      ...
      <NuxtLink
        :to="post._path"
        class="mt-auto text-primary-600 dark:text-primary-400 font-medium inline-flex items-center hover:underline"
      >
